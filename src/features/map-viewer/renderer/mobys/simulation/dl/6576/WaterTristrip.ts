...
export class WaterTristripMobyClass extends MobyClass {
  private readonly layers: WaterTristripLayer[] = [];
  private readonly textures = new Set<THREE.Texture>();

  static async create(context: MobyClassContext): Promise<WaterTristripMobyClass | null> {
    ...
    const configs = context.instances.flatMap((instance): WaterTristripConfig[] => {
      const pvar = parseWaterTristripPvar(instance.pvar?.data);
    ...
    await water.load(configs, await loadFxTextureUrls(context));
  ...
  private constructor(context: MobyClassContext) {
  ...
  private async load(configs: WaterTristripConfig[], textureUrls: Map<number, string>): Promise<void> {
    const loader = new THREE.TextureLoader();
    for (const [configIndex, config] of configs.entries()) {
      this.addMesh(
        `dl_water_tristrip_${configIndex}_underlay`,
        createWaterTristripGeometry(
          config.spline,
          1,
          1,
      ...
      const sourceTexture = textureUrl ? await loadTexture(loader, textureUrl) : null;
      if (!sourceTexture) {
        continue;
      }

      for (const [pass, passConfig] of waterTristripColorPasses.entries()) {
        ...
        const name = `dl_water_tristrip_${configIndex}_overlay_${pass}`;
        this.addMesh(
          name,
          createWaterTristripGeometry(
            config.spline,
